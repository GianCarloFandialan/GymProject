...
import MSCCardModifyButton from "./MSCCardModifyButton";
import MCSModal from "./MCSModal";
import MCSModalSuccess from "./MCSModalSuccess";
...
function MCSCard({ lesson, classes, setClasses }) {
  ...
  return (
    <div className="flex flex-col overflow-hidden border-2 border-black p-2 rounded-xl">
      ...
      <MSCCardModifyButton setOpenModal={setOpenModal} />
      ...
      {openModalSuccess && (
        <MCSModalSuccess setOpenModalSuccess={setOpenModalSuccess} />
      )}
    </div>
  );
}
