function ChatSidebar({ trainers, setChatter }) {
  return (
    <>
      <div className="w-screen md:w-[40vw] lg:w-[27vw] absolute overflow-y-scroll h-full scrollbar-thin scrollbar-track-white scrollbar-thumb-gray-300">
          ...
          {trainers.map((trainer) => {
            return (
              <div key={trainer._id + "chat-normal"}>
                ...
                <li
                  className="flex-1 cursor-pointer md:hidden block"
                  key={trainer._id + "chat-sm"}
                >
                  <div className="flex py-11 px-5 border-black border-b-[1px] md:py-12 bg-white">
                    <div className="w-16 h-16">
                      <img
