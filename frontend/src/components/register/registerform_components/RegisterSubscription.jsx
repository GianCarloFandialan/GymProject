...
function RegisterSubscription( { newUser, setNewUser } ) {
  ...
  useEffect(() => {
    setNewUser({ ...newUser, Subscription: {id:selectedSubscription, start:date} })
        ...
        <div className="relative items-center col-span-6 mb-6 -mx-8">
          <div className="justify-center space-y-4 sm:space-y-0 sm:grid sm:grid-cols-2 sm:gap-6 xl:mx-0 xl:grid-cols-3">     
            {subscriptions.map((subscription, index) => {
              if (index % 2 === 0) {
                return(
                  <RegisterSubWhite subscription={subscription} key={subscription._id} setSelectedSubscription={setSelectedSubscription} selectedSubscription={selectedSubscription}/>
                )
              } else {
                return (
                  <RegisterSubBlack subscription={subscription} key={subscription._id} setSelectedSubscription={setSelectedSubscription} selectedSubscription={selectedSubscription}/>
                )
              }
            })}
          </div>
